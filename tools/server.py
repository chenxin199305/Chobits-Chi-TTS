...
参数与 api_v2.py 完全一致:
  python tools/server.py -c GPT_SoVITS/configs/tts_infer_chi.yaml -a 0.0.0.0 -p 9880
...
# 可用 CHI_TTS_REF_AUDIO / CHI_TTS_REF_TEXT_FILE 覆盖默认参考音频与参考文本
REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_ref_text_file = os.environ.get("CHI_TTS_REF_TEXT_FILE",
                                os.path.join(REPO_ROOT, "models", "ref_text.txt"))
try:
...
VOICES = {
    "chi": {
        "ref_audio_path": os.environ.get("CHI_TTS_REF_AUDIO",
                                         os.path.join(REPO_ROOT, "models", "ref_audio.wav")),
...
async def openai_models():
    return {"object": "list", "data": [
        {"id": TTS_MODEL, "object": "model", "created": 0, "owned_by": "chobits-chi"}]}
...
async def openai_audio_speech(request: Request):
    ...
    voice = str(body.get("voice") or "chi")
